import { Shield, LayoutDashboard, BookOpen, Instagram } from 'lucide-react'

export default function Header({ onAnalyzeModels, onNavigate, onEnterApp, onEnterDashboard, onEnterTraining, onEnterInstagramLive }) {
  return (
    <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {onEnterInstagramLive && (
            <button
              onClick={onEnterInstagramLive}
              className="text-gray-600 border border-gray-200 text-sm px-4 py-2 rounded-lg hover:bg-gray-50 transition hidden md:block"
            >
              <Instagram className="w-4 h-4 inline-block mr-1 mb-0.5" />
              Live IG
            </button>
          )}
          {onEnterApp && (
